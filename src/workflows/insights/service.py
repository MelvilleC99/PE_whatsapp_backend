...
class InsightsService:
    ...
    def __init__(self):
        ...
        self.generator = None  # Lazy load only when needed
    
    def _get_generator(self):
        """Lazy load the insight generator (connects to PostgreSQL)"""
    ...
    def generate_and_save_insights(self, user_id: str, use_mock: bool = False) -> Dict:
        ...
        try:
            # Generate insights
            if use_mock:
                insights_data = self._get_generator().generate_mock_insights()
            else:
                insights_data = self._get_generator().generate_insights_for_user(user_id)
