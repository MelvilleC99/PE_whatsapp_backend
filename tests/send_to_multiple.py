"""
Send insights_summary template to multiple recipients
"""
import sys
from pathlib import Path

# Add project root to Python path
project_root = Path(__file__).parent.parent
sys.path.insert(0, str(project_root))

import requests
from src.config import settings
from src.utils import format_phone_number

def send_insights_summary(
    to: str,
    name: str,
    leads: str,
    portal: str,
    offer: str,
    sale: str,
    revenue: str,
    commission: str
):
    """Send the insights_summary template"""
    formatted_phone = format_phone_number(to)
    
    url = f"{settings.meta_graph_api_url}/{settings.whatsapp_phone_number_id}/messages"
    headers = {
        "Authorization": f"Bearer {settings.whatsapp_access_token}",
        "Content-Type": "application/json"
    }
    
    payload = {
        "messaging_product": "whatsapp",
        "to": formatted_phone,
        "type": "template",
        "template": {
            "name": "insights_summary",
            "language": {"code": "en"},
            "components": [
                {
                    "type": "body",
                    "parameters": [
                        {"type": "text", "text": str(name)},
                        {"type": "text", "text": str(leads)},
                        {"type": "text", "text": str(portal)},
                        {"type": "text", "text": str(offer)},
                        {"type": "text", "text": str(sale)},
                        {"type": "text", "text": str(revenue)},
                        {"type": "text", "text": str(commission)}
                    ]
                }
            ]
        }
    }
    
    try:
        response = requests.post(url, headers=headers, json=payload)
        response.raise_for_status()
        
        result = response.json()
        msg_id = result.get('messages', [{}])[0].get('id', 'N/A')
        
        print(f"✅ Sent to {name} ({formatted_phone}) - Message ID: {msg_id}")
        return result
        
    except Exception as e:
        print(f"❌ Failed to send to {name} ({formatted_phone}): {e}")
        raise

if __name__ == "__main__":
    print("📤 Sending insights_summary to test recipients...\n")
    
    # List of recipients with their data
    recipients = [
        {
            "to": "27727377590",
            "name": "Melville",
            "leads": "150",
            "portal": "P24 - 55%",
            "offer": "5",
            "sale": "3",
            "revenue": "R12.5mil",
            "commission": "R200k"
        },
        {
            "to": "27765144639",
            "name": "Leon",
            "leads": "180",
            "portal": "Property24 - 60%",
            "offer": "8",
            "sale": "5",
            "revenue": "R15mil",
            "commission": "R250k"
        }
    ]
    
    success_count = 0
    fail_count = 0
    
    for recipient in recipients:
        try:
            send_insights_summary(**recipient)
            success_count += 1
        except:
            fail_count += 1
    
    print(f"\n{'='*60}")
    print(f"✅ Sent: {success_count}")
    print(f"❌ Failed: {fail_count}")
    print(f"📱 Check WhatsApp!")
